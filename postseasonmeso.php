...
<br>
<h4 style="color:red;">MesoCycle Stage:</h4>
...
<form  action="postseasonmeso.php" method="post">
...
if(isset($_POST['postbtn'])){
    ...
    $season = 'postseason';
        ...
        $sql1="SELECT * FROM mesoscores where phonenumber = '$phonenumber' and season = '$season' Limit 1";
        ...
        if($queryResults) {
            $sql = "UPDATE mesoscores set firstscore = '$first',finalscore = '$final' where phonenumber= '$phonenumber' and season = '$season'";
            ...
            if($res ==1){
                ...
                if($queryResults) {
                    while($row = mysqli_fetch_assoc($result)) {
                        ...
                        if($firstscore == 'best' && $finalscore == 'best')
                        {
                            echo "<script>alert('You passed the test.Click Ok to proceed to the next level')</script>";
                    
                            echo "<script>location.replace('postseasonmacro.php')</script>"; 
                        }else{
                            echo "<script>alert('You failed.Click Ok to repeat.')</script>";
                            echo "<script>location.replace('postseasonmeso.php')</script>"; 
                        }
            ...
            $sql = "INSERT INTO mesoscores (season,phonenumber,firstscore,finalscore) VALUES ('$season','$phonenumber','$first','$final')";
		$res = mysqli_query($con,$sql);
